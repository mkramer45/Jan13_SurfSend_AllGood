...
while True:
	try:
		...
		# in this case, let's grab info for Green Harbor emails we are going to send
		cur2 = cursor.execute("select * from surfmaster2 where surfmaster2.time_ not in ('3am','12am','9pm') and SurfMaster2.SwellSizeFt >= 3 and SurfMaster2.SwellIntervalSec >= 8 and SurfMaster2.WindMPH <= 14 and SurfMaster2.beach_name = 'Green Harbor' and SurfMaster2.WindDescription not like '%Onshore%' or '%trong%' limit 2;")
		info2 = cur2.fetchall()
		conn.commit()
		conn.close()
		conn.close()
		...
		# Message Example: Green Harbor: 12ft at 9sec on 2018-10-29 ~ 9am
		...
		# so, again, for this - we are using Green Harbor
		conn5 = sqlite3.connect('SurfSend.db')
		cursor5 = conn5.cursor()
		cur5 = cursor5.execute('SELECT email FROM ArtistMonitor WHERE DJName = "Green Harbor"')
		info5 = cur5.fetchall()
		conn5.commit()
		conn5.close()
		conn5.close()
