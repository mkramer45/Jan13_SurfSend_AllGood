...
while True:
	try:
		...
		cur2 = cursor.execute("select * from surfmaster2 where surfmaster2.time_ not in ('3am','12am','9pm') and SurfMaster2.SwellSizeFt >= 3 and SurfMaster2.SwellIntervalSec >= 8 and SurfMaster2.WindMPH <= 14 and SurfMaster2.beach_name = 'Nantasket' and SurfMaster2.WindDescription not like '%Onshore%' or '%trong%' limit 2;")
		info2 = cur2.fetchall()
		conn.commit()
		conn.close()
		conn.close()
		...
		cur5 = cursor5.execute('SELECT email FROM ArtistMonitor WHERE DJName = "Nantasket"')
		info5 = cur5.fetchall()
		conn5.commit()
		conn5.close()
		conn5.close()
